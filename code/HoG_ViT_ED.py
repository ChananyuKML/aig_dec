...
transform = transforms.Compose([
    transforms.Resize((256, 256)),  # <- resized to 256x256
...
def get_dataloaders(data_root, batch_size=32, num_workers=2):
    train_dataset = NPYDataset(os.path.join(data_root, 'train'), transform = transform)
    valid_dataset = NPYDataset(os.path.join(data_root, 'valid'), transform = transform)
    test_dataset  = NPYDataset(os.path.join(data_root, 'test'), transform = transform)
...
def train_model(model, train_loader, valid_loader, batch_size=32, num_epochs=10, lr=1e-4, device='cpu'):
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), f'../checkpoints/HoG_ViT_Normalized/ckpt_{epoch+1}.pth')
...
torch.save(model.state_dict(), f'../model/HoG_ViT_Normalized/model_from_ckpt_{num_epochs}.pt')